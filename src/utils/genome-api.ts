...
export async function analyzeVariantWithAPI({
  ...
  const API_URL = process.env.NEXT_PUBLIC_ANALYZE_SINGLE_VARIANT_BASE_URL;

  if (!API_URL) {
    throw new Error(
      "API URL is not configured. Please set NEXT_PUBLIC_ANALYZE_SINGLE_VARIANT_BASE_URL."
    );
  }
  ...
  const response = await fetch(API_URL, {
    ...
    // The data goes here, converted to a JSON string.
  ...
  if (!response.ok) {
    ...
    throw new Error(errorData.message || "Failed to analyze variant.");
  }
